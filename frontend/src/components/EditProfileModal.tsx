...
const EditProfileModal: React.FC<EditProfileModalProps> = ({ isOpen, onClose, userId, onProfileUpdate }) => {
    ...
    const fetchProfileData = async () => {
        try {
            // Fetch own details and profile picture in parallel using unified user routes
            const [detailsResponse, picResponse] = await Promise.all([
                getData('/user/details'),
                getData(`/user/profile-picture/${userId}`)
            ]);

            if (detailsResponse.status) {
                setUsername(detailsResponse.data.username || '');
            }
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const response = await postData('/user/profile-picture', formData);
            if (response.status) {
    ...
    const handleUsernameUpdate = async () => {
        ...
        try {
            const response = await postData('/user/profile/username', {
    ...
    const handleRequestOtp = async () => {
        ...
        try {
            // Auth token verified by the backend — no body needed
            const response = await getData('/user/password/request-otp');
            if (response.status) {
    ...
    const handlePasswordChangeWithOtp = async () => {
        ...
        try {
            const response = await postEncryptedData('/user/password/change', {
                            ...
                            <>
                                ...
                                <div className="flex gap-3">
                                    <button
                                        ...
                                        disabled={isLoading}
                                        className="flex-1 py-2 rounded-lg font-medium hover:opacity-80 transition-opacity"
                                        style={{ backgroundColor: theme === 'dark' ? '#444' : '#ddd', color: textColor }}
                                    >
                                        Back
                                    </button>
                                    <button
                                        ...
                                        disabled={isLoading}
                                        className="flex-1 py-2 rounded-lg font-medium text-white hover:opacity-80 transition-opacity"
                                        style={{ backgroundColor: isLoading ? '#666' : '#4CAF50' }}
                                    >
