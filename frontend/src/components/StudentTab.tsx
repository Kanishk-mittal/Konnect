    ...
    <>
          ...
          onClick={() => {
            ...
            if (context === 'club') {
              if (isBlocked) {
                ...
                // Block student for club
                postEncryptedData('/club/students/block-bulk', {
                  students: [{ roll: rollNumber }]
                })
                  .then(() => {
                    alert(`Student ${name} blocked successfully!`);
                    window.location.reload();
                  })
                  .catch((error: any) => {
                    console.error('Error blocking student:', error);
                    alert(`Failed to block student: ${error.response?.data?.message || 'Unknown error'}`);
                  });
              }
            } else {
            ...
            <button
              onClick={() => {
                if (context === 'club' && position !== undefined) {
                  ...
                  setIsEditModalOpen(true); // Open EditStudentModal for admin users
                }
              }}
              className="p-2 rounded-md hover:opacity-80 transition-opacity"
              style={{ backgroundColor: 'rgba(255, 255, 255, 0.2)' }}
      ...
      {context === 'admin' && (
        <EditStudentModal
          ...
          studentEmail={''} // Pass email if available
          onSuccess={() => {
