...
import { decryptRSA, encryptRSA } from '../../encryption/RSA_utils';
import { decryptAES, encryptAES, generateAESKey } from '../../encryption/AES_utils';
...
export const userLoginController = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        // Find user by college code and roll number
        const user = await UserModel.findOne({ 
            college_code: loginData.collegeCode,
            roll_number: loginData.rollNumber
        });
        ...
        // Set JWT token
        const jwtPayload = { type: 'user', id: user._id.toString() };
        ...
        if (encryptedData.publicKey) {
            try {
                ...
                const userPrivateKey = user.private_key as string;
                ...
                const encryptedPrivateKey = encryptAES(userPrivateKey, responseAesKey);
                const encryptedId = encryptAES(userId, responseAesKey);
