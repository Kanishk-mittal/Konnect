...
# Create year groups
for year in years:
    Group(name=year, description=f"Students admitted in {year}").to_db(db)
    ...
    for batch in range(1, batches_per_year + 1):
        ...
        Group(name=batch_name, description=f"Batch {batch} of {year}").to_db(db)
        print(f"Created group {batch_name}")
    
    # Create department groups for each year
    for dept in departments:
        dept_name = f"{year}-{dept}"
        Group(name=dept_name, description=f"{dept} students of {year}").to_db(db)
        print(f"Created group {dept_name}")

# Create college-wide group
Group(name="IIIT-Kottayam", description="All students of IIIT Kottayam").to_db(db)
