...
class Config:
    ...
    SENDER_EMAIL = os.getenv("psr6mummadisetty@gmail.com")  # Replace with your email address
    SENDER_PASSWORD = os.getenv("PavanSRK")  # Replace with your email password

    # Secret key for sessions and CSRF protection (Ensure to change this to a random value in production)
    SECRET_KEY = os.getenv("secret") # Change to a strong secret key in production

    # JWT related configurations
    JWT_SECRET_KEY = os.getenv("secret")  # JWT secret key
